...
impl Orchestrator {
    ...
    pub async fn handle_classification_with_gen(
        ...
        println!("Starting handle_classification_with_gen task with request_id: {}", request_id);
        let task_handle = tokio::spawn(async move {
            ...
            println!("Input detections: {:?}", input_detections);
            if let Some(mut input_detections) = input_detections {
                ...
                .await?;
                println!("Tokenized input with token count: {}", input_token_count);
                ...
                .await?;
                println!("Generated text results: {:?}", generation_results);
                ...
                println!("Output detections: {:?}", output_detections);
        ...
        match task_handle.await {
            ...
            Err(error) => {
                ...
                error!(%request_id, %error, "task failed");
                println!("Task panicked or was cancelled with error: {:?}", error);
                Err(error)
            }
        }
    }
    ...
    pub async fn handle_generation_with_detection(
        ...
        println!("Starting handle_generation_with_detection task with request_id: {}", task.request_id);
        let ctx = self.ctx.clone();
        let headers = task.headers;
        let task_handle = tokio::spawn(async move {
    ...
    pub async fn handle_text_content_detection(
        ...
        println!("Starting handle_text_content_detection task with request_id: {}", task.request_id);
    ...
    pub async fn handle_context_documents_detection(
        ...
        println!("Starting handle_context_documents_detection task with request_id: {}", task.request_id);
        let ctx = self.ctx.clone();
        let headers = task.headers;
        let task_handle = tokio::spawn(async move {
            // call detection
    ...
    pub async fn handle_generated_text_detection(
        ...
        println!("Starting handle_generated_text_detection task with request_id: {}", task.request_id);
        let ctx = self.ctx.clone();
        let headers = task.headers;
    ...
    pub async fn handle_chat_detection(
        ...
        println!("Starting handle_chat_detection task with request_id: {}", task.request_id);
        let ctx = self.ctx.clone();
        let headers = task.headers;
...
async fn output_detection_task(
    ...
    println!("Starting output_detection_task");
    let text_with_offsets = apply_masks(generated_text, None);
    let chunker_ids = get_chunker_ids(ctx, detectors)?;
    let chunks = chunk_task(ctx, chunker_ids, text_with_offsets).await?;
    let detections = detection_task(ctx, detectors, chunks, headers).await?;
    println!("Finished output_detection_task");
    Ok((!detections.is_empty()).then_some(detections))
}
...
async fn detection_task(
    ...
    println!("Starting detection_task");
    ...
    let results = try_join_all(tasks)
        ...
        .flatten()
        .collect::<Vec<_>>();
    println!("Finished detection_task");
    Ok(results)
}
...
pub async fn detect_content(
    ...
    println!("Starting detect_content");
    let detector_id = detector_id.clone();
    let threshold = detector_params.threshold().unwrap_or(default_threshold);
    let contents: Vec<_> = chunks.iter().map(|chunk| chunk.text.clone()).collect();
    let response = if contents.is_empty() {
    ...
    let results = chunks
        ...
        .collect::<Vec<_>>();
    println!("Finished detect_content");
...
pub async fn detect_for_generation(
    ...
    println!("Starting detect_for_generation");
    let detector_id = detector_id.clone();
    ...
    println!("Finished detect_for_generation");
...
pub async fn detect_for_chat(
    ...
    println!("Starting detect_for_chat");
    let detector_id = detector_id.clone();
    let threshold = detector_params.threshold().unwrap_or(
        detector_params.threshold().unwrap_or(
            ctx.config
    ...
    println!("Finished detect_for_chat");
    Ok::<Vec<DetectionResult>, Error>(response)
}
...
pub async fn detect_for_context(
    ...
    println!("Starting detect_for_context");
    let detector_id = detector_id.clone();
    let threshold = detector_params.threshold().unwrap_or(
        detector_params.threshold().unwrap_or(
            ctx.config
    ...
    println!("Finished detect_for_context");
    Ok::<Vec<DetectionResult>, Error>(response)
}
...
pub async fn chunk_parallel(
    ...
    println!("Starting chunk_parallel");
    let chunks = stream::iter(text_with_offsets)
        ...
        .flatten()
        .collect::<Vec<_>>();
    println!("Finished chunk_parallel");
...
async fn generate(
    ...
    let client_result = ctx.clients.get_as::<GenerationClient>("generation");
    let client = match client_result {
        Some(client) => {
            println!("Successfully retrieved GenerationClient for 'generation'");
            client
        },
        None => {
            println!("Failed to retrieve GenerationClient for 'generation'");
            return Err(Error::Other("Failed to retrieve GenerationClient".into()));
        }
    };

    let result = client
        .generate(model_id.clone(), text.clone(), params.clone(), headers.clone())
        .await
        .map_err(|error| {
            println!("Generate request failed for `{}` with text `{}` and params `{:?}`: {:?}", model_id, text, params, error);
    ...
    match &result {
        Ok(response) => println!("Generate response: {:?}", response),
...
mod tests {
    ...
    async fn test_tgis_generate_with_default_params() {
        println!("Starting test_tgis_generate_with_default_params");
        ...
        println!("Finished test_tgis_generate_with_default_params");
    }
    ...
    async fn test_handle_detection_task() {
        println!("Starting test_handle_detection_task");
        ...
        println!("Finished test_handle_detection_task");
    }
    ...
    async fn test_detect_when_detector_returns_503() {
        println!("Starting test_detect_when_detector_returns_503");
        let generation_client = GenerationClient::tgis(TgisClient::faux());
        let mut detector_client = TextContentsDetectorClient::faux();
        ...
        println!("Finished test_detect_when_detector_returns_503");
    }

    #[tokio::test]
    async fn test_handle_detection_task_with_whitespace() {
        println!("Starting test_handle_detection_task_with_whitespace");
        let generation_client = GenerationClient::tgis(TgisClient::faux());
        let mut detector_client = TextContentsDetectorClient::faux();
        ...
        println!("Finished test_handle_detection_task_with_whitespace");
    }

    #[tokio::test]
    async fn test_detect_for_generation() {
        println!("Starting test_detect_for_generation");
        let generation_client = GenerationClient::tgis(TgisClient::faux());
        ...
        println!("Finished test_detect_for_generation");
    }

    #[tokio::test]
    async fn test_detect_for_generation_below_threshold() {
        println!("Starting test_detect_for_generation_below_threshold");
        let generation_client = GenerationClient::tgis(TgisClient::faux());
        let mut detector_client = TextGenerationDetectorClient::faux();
        ...
        println!("Finished test_detect_for_generation_below_threshold");
    }
}